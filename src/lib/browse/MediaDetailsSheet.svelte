...
{#if isDesktop}
  {#if open && shown}
    ...
    <!-- Desktop Side Drawer Content -->
    <div
      class="fixed right-0 top-0 bottom-0 z-50 flex h-full w-[460px] max-w-full flex-col border-l border-app-separator bg-app-surface text-app-label shadow-2xl"
      transition:fly={{ x: 460, duration: 300, easing: cubicOut }}
    >
      <div class="flex items-center justify-between px-6 pb-4 pt-6 border-b border-app-separator">
        <span
      ...
      <div class="min-h-0 flex-1 overflow-y-auto px-6 py-6 no-scrollbar">
        <div class="flex gap-4">
          <div
            class="h-44 w-32 shrink-0 overflow-hidden rounded-[10px] bg-app-surface-hover shadow-md"
          >
            {#if poster}
              <img
                src={poster}
        ...
        {:else if details}
          ...
          {#if details.cast.length}
            <section class="mt-6">
              ...
              <div class="no-scrollbar -mx-6 mt-3 flex gap-3 overflow-x-auto px-6 pb-1">
      ...
      <div
        class="border-t border-app-separator bg-app-surface px-6 py-4 shadow-[0_-12px_24px_rgb(0_0_0_/_0.28)]"
      >
        <button
          type="button"
          class="flex h-12 w-full items-center justify-center gap-2 rounded-[10px] bg-apple-green text-base font-extrabold text-apple-black transition-transform active:scale-[0.98] hover:scale-[1.01]"
          onclick={watchTitle}
        >
          <svg
            class="h-5 w-5"
